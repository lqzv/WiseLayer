...
wiselayer 在 Cesium 原生 Provider 之上提供一层**统一的图层抽象**，让 WMS、WMTS 和天地图可以用相同的方式创建、管理和交互，而不重复实现 OGC 协议细节。

```
用户配置 (LayerOptions)
        │
        ▼
   createLayer()  ──►  WmsLayer / WmtsLayer / TiandituLayer
        │                    │
        │                    ▼
        │           toImageryProvider()
        │                    │
...
│   └── TiandituLayer.ts    # 天地图 WMTS 实现
├── factory/
│   └── createLayer.ts      # 工厂方法、批量添加
└── utils/                  # 工具函数
    ├── crs.ts              # WMS crs/srs 版本适配
    ├── tianditu.ts         # 天地图 URL / 切分方案 / 级别映射
    ├── url.ts              # URL 参数合并
    ├── camera.ts           # 相机定位
...
## 数据流：TiandituLayer.toImageryProvider

1. 解析 `crs`（默认 EPSG:3857）与 `imageType`（默认 vector）
2. 通过 `buildTiandituWmtsServiceInfo` 生成服务 URL 与 TileMatrixSet
3. 选择切分方案：EPSG:3857 → WebMercatorTilingScheme，EPSG:4326 → GeographicTilingScheme
4. EPSG:4326 时生成 tileMatrixLabels（天地图级别从 1 起编，Cesium 从 0 起编）
5. 创建 WebMapTileServiceImageryProvider 并传入子域名 t0–t7
...
当前版本聚焦 WMS / WMTS / 天地图影像图层，保持 API 简洁。
