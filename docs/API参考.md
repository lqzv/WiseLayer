...
不建议直接实例化，通过 `WmsLayer` / `WmtsLayer` 或 `createLayer` 使用。
...
```typescript
// 类型
export type { BaseLayerOptions, CenterAtOptions, LayerOptions, LayerType, WmsLayerOptions, WmtsLayerOptions, ILayer };

// 类
export { BaseLayer, WmsLayer, WmtsLayer };
